...
Des weiteren gleichen sich die Verfahren des DHKE und die Verschlüsselung
mittels DHKE, wobei Bob den öffentlichen Schlüssel von Alice beim
Schlüsselserver erfragt und an der Stelle der Erzeugung des geheimen Schlüssels
$K$ bei DHKE wird stattdessen die Nachricht $m$ zum Ciphertext $c$
verschlüsselt, mit $(c \equiv A^b \cdot m \Mod{p}$, und zusammen mit dem
öffentlichen Schlüssel $B$ von Bob an ein Postfach von Alice gesendet.
Alice entschlüsselt die Nachricht dann mit $m = B^{-a}\cdot c \Mod{p}$, was nach
dem kleinen Satz von Fermat (siehe Abschnitt~\ref{def:erzeuger}) kongruent ist
zu $m = B^{(p-1)-a} \cdot c \Mod{p}$.
