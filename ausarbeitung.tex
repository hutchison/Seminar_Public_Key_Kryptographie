...
Zum Schluss noch ein Satz zur Kongruenzrechnung, der später beim
Diffie-Hellman-Schlüsselaustausch (Abschnitt~\ref{sub:dhke_proof}) benötigt
wird:
\begin{satz}\label{satz:potenzen_bei_kongruenzen}
  Seien $a \in \Z$ und $k, u, n \in \N \setminus \{ 0 \}$, dann gilt
  ${(a^k \Mod{n})}^u \equiv a^{k\cdot u} \mod n$.
  \begin{proof}
    \todo{quelle einfügen [Menezes, S.68 2.112(V)]}
    Seien $a, a_1, b, b_1 \in \Z$ und $n \in \N \setminus \{0\}$ und gelte $a
    \equiv a_1 \Mod{n}$ und $b \equiv b_1 \Mod{n}$.
    \begin{align*}
      a \cdot b & \equiv a_1 \cdot b_1 \mod n
    \end{align*}
    Angenommen $a=b$, also
    \begin{align*}
    ...
    dann folgt
    \begin{align*}
      a^k & \equiv a_1^k \mod n \quad \text{ mit } k \in \N \setminus \{ 0 \}
    \end{align*}
    und daraus
    \begin{align*}
      {(a^k)}^u & \equiv {(a_1^k)}^u \mod n \quad \text{ mit } u \in \N \setminus \{ 0 \}
    \end{align*}
    ...
    und
    \begin{align*}
      {(a_1^k \Mod{n})}^u & \equiv a_1^{k\cdot u} \mod n
    \end{align*}
  \end{proof}
\end{satz}
...
möglich sein wird (siehe Abschnitt~\ref{sub:komplexitaet_naiver_algorithmus}).
...
Mit der Kongruenz aus Satz~\ref{satz:potenzen_bei_kongruenzen} lässt sich nun
einfach zeigen, dass die Schlüssel $K_1$ von Alice und $K_2$ von Bob kongruent
sind:
\begin{align*}
  K_1 & \equiv B^a \mod p\\
  & \equiv {(g^b)}^a \mod p\\
...
\begin{beispiel}[Diffie-Hellman-Merkle Schlüsselaustausch]\label{bsp:dhke}
  ...
  $K_2 \equiv A^b \mod p$, $K = K_1 = K_2$, ermitteln zu können.
\end{beispiel}
...
anderen Kommunikationspartner (Alice) über einen unsicheren Kanal.
...
\begin{algorithm}\label{alg:dlog_naive}
...
Im ersten Schritt wählt Alice eine ausreichend große Primzahl $p$, welche den \todo{bekannte Schwachstellen rausnehmen?}bekannten Fallstricken des Verfahrens ausweicht, z.B. mit $p = c \cdot q+1$, $c \in \N\backslash\{0\}$, $q$ prim, $q$ möglichst groß (etwa $p = 2 \cdot q + 1$). Ausreichend groß bedeutet hierbei, dass die Bitlänge $n$ der Primzahl $p$ so groß ist, dass die Berechnung des diskreten Logarithmus $\text{DL}(y,g,p)$ mit bekannten Algorithmen und aktueller Hardware auf absehbare Zeit nicht möglich sein wird (siehe Abschnitt \ref{sub:komplexitaet_naiver_algorithmus}). Zur Zeit (Jahr 2017) kann für $p$ eine Bitlänge von $n_p = 2048$ und für $q$ eine Bitlänge von $n_q = 256$ als sicher \todo{source: https://security.stackexchange.com/questions/112313/what-is-the-current-security-status-of-diffie-hellman-key-exchange}angenommen werden. Weiterhin sucht Alice einen \todo{ist die Formulierung bei Erzeuger korrekt?}Erzeuger $g \in \Z_p^*$, $(2 \leq g \leq p-2)$, welcher $g^k \mod p$, $k \in \N \backslash \{0\}$, zyklisch ohne Untergruppen auf $\Z_p^*$ abbildet (siehe Abschnitt \ref{def:erzeuger}). Dann wählt Alice ihren geheimen privaten Schlüssel $a \in \Z_p^*$, $(1 \leq a \leq p-2)$,  berechnet ihren öffentlichen Schlüssel $A$ mit $A \equiv g^a \mod p$ und sendet ($g$, $p$, $A$) über den unsicheren Kanal an Bob.
...
\label{sub:enc_with_dhke}
Ein Kommunikationspartner (Bob) schickt eine geheime Nachricht $m$ an einen anderen Kommunikationspartner (Alice) über einen unsicheren Kanal durch Verschlüsselung mittels \textbf{D}iffie-\textbf{H}ellman-\textbf{K}ey-\textbf{E}xchange (DHKE). Bevor, möglicherweise lange bevor, Bob eine Nachricht an Alice schicken möchte, muss diese Ihren öffentlichen Schlüssel in geeigneter Weise auf einem Schlüsselserver veröffentlichen, wie z.B. \url{https://sks-keyservers.net}. Des weiteren gleichen sich die Verfahren des DHKE und die Verschlüsselung mittels DHKE, wobei Bob den öffentlichen Schlüssel von Alice beim Schlüsselserver erfragt und an der Stelle der Erzeugung des geheimen Schlüssels $K$ bei DHKE wird stattdessen die Nachricht $m$ zum Ciphertext $c$ verschlüsselt, mit $(c \equiv A^b \cdot m \mod p$, und zusammen mit dem öffentlichen Schlüssel $B$ von Bob an ein Postfach von Alice gesendet. Alice entschlüsselt die Nachricht dann mit $m = B^{-a}\cdot c  \mod p$, was nach dem kleinen Satz von Fermat (siehe Abschnitt \ref{def:erzeuger}) kongruent ist zu $m = B^{(p-1)-a}\cdot c  \mod p$.
